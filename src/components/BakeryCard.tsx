  ...
  return (
    <button
      ...
      className={`text-left w-full p-3 bg-white rounded-xl shadow-sm hover:shadow-md transition-all border flex gap-3 ${
        featured ? 'border-yelp-300 ring-1 ring-yelp-200' : 'border-stone-200'
      }`}
    >
      {/* Number Badge */}
      {listNumber !== undefined && (
        <div className={`flex-shrink-0 w-7 h-7 ${typeColor} text-white rounded-full flex items-center justify-center text-sm font-bold`}>
          {listNumber}
        </div>
      )}
